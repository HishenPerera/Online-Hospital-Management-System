...
<body style="background-image: url('${pageContext.request.contextPath}/assets/images/repbg.jpg')">
	...
	<h1 style="color:white; font-size:45px; font-weight:bold; text-shadow: 2px 2px 4px rgba(0, 0, 0, 0.5); text-align:center;">Manage All Attendance</h1>
    <table class="table" style="margin: 0 auto; width:1250px; margin-top:20px;">
    <tr class="table-primary">
        <th>ID</th>
        <th>Doctor Name & Speciality</th>
        <th>Attend Date</th>
        <th>Morning Hours (6.00 a.m - 8.00 a.m)</th>
        <th>Night Hours (6.00 p.m - 9.00 p.m)</th>
        <th>Action</th>
    </tr>
    <c:forEach items="${attends}" var="attend">
    <tr class="table-light">
        <td>${attend.id}</td>
        <td>${attend.attendDetails}</td>
        <td>${attend.date}</td>
        <td>${attend.morning}</td>
        <td>${attend.night}</td>
        <td>
        	<a href="${pageContext.request.contextPath}/pages/staff/receptionist/updateAttend.jsp?id=${attend.id}" class="btn btn-primary btn-sm">Update</a>
            <a href="${pageContext.request.contextPath}/deleteAttend?id=${attend.id}" class="btn btn-danger btn-sm" onclick="return confirm('Are you sure you want to delete this attendance?')">Delete</a>
        </td>
    </tr>
</c:forEach>
</table>
